<?php
    if(isset($_POST['employee'])){
        ...
        // ---------------------------------------------------------------------
        // 🔑 1. Recibir el ID de la sede del POST usando 'office_id'
        // Se asume que el ID de la oficina viene en $_POST['office_id'].
        ...
        if($query->num_rows > 0){
            ...
            if($status == 'in'){
                ...
                if($query->num_rows > 0){
                    ...
                    //
                    // -------------------------------------------------------------------------------------------------
                    // 🔑 2. Modificar la sentencia INSERT para incluir el campo `sede` con el valor de $office_id
                    // -------------------------------------------------------------------------------------------------
                    $sql = "INSERT INTO attendance (employee_id, date, time_in, status, sede) VALUES ('$id', '$date_now', NOW(), '$logstatus', '$office_id')";
                    
                    if($conn->query($sql)){
                        $output['message'] = 'Llegada: '.$row['firstname'].' '.$row['lastname'] . ' (Office ID: ' . $office_id . ')';
                    }
                    else{
                        $output['error'] = true;
                ...
                // La lógica de registro de salida 'out' permanece sin cambios
